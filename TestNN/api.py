    ...
    def get_landmarks(self, input_image, all_faces=False):
        ...
        if len(detected_faces) > 0:
            ...
            for i, d in enumerate(detected_faces):
                ...
                print("D::sizeof(image) before crop:\n", image.shape)
                inp = crop(image, center, scale)
                print("D::inp after crop:\n", type(inp), inp.size)
                inp = torch.from_numpy(inp.transpose(
                    (2, 0, 1))).float().div(255.0).unsqueeze_(0)
                print("D::inp after from_numpy:\n", type(inp), inp.size())
...
def face2d(filename):
    ...
    print(filename, input.shape, type(input[0][0]))
#    print(input)       
    ...
    if not preds is None:
        ...
        for pred in preds[0]:
            input[int(pred[1])-1:int(pred[1])+1,int(pred[0]-1):int(pred[0]+1)][:] = 255
        return input
    else:
        return None

if __name__ == "__main__":
    basedir = r'/home/kg/face-alignment/'
    inputdir = basedir + r'depth_img_uint8/'
    ...
    for dir in dirs:
        if os.path.isdir(inputdir + dir):
            ...
            for img in files:
                if not os.path.isdir(inputdir+dir+'/'+img) and img[-3:] == 'jpg':
                    output = face2d(inputdir + dir + '/' + img)
                    break
            break